...
public:
     ...
     */
    static std::vector<DirectoryFile> getFilesIn(std::string path);
    ...
    /**
     * Calculates the SHA512 checksum for the specified file path. If it fails (for example because the file doesn't
     * exists), it'll return false.
     */
    static bool calculateSHA512(std::string path, char* out);

    /**
     * Calculates the SHA512 checksum for the specified stream. If it fails, it'll return false.
     */
